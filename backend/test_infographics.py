...
class InfographicTests(unittest.TestCase):
    ...
    def test_deanston_sql_contains_valid_localized_timeline(self):
        sql_file = Path(__file__).with_name("migrations") / "006_seed_deanston_infographic.sql"
        sql = sql_file.read_text(encoding="utf-8")
        match = re.search(r"\$deanston_data\$\s*(.*?)\s*\$deanston_data\$", sql, re.DOTALL)
        self.assertIsNotNone(match)
        source_steps = json.loads(match.group(1))
        headings = re.findall(r"^\s*\((\d+), '([^']+)', '([^']+)', '([^']+)'\),?$", sql, re.MULTILINE)
        self.assertEqual(len(source_steps), 21)
    ...
    def test_glenfiddich_sql_contains_valid_localized_timeline(self):
        sql_file = Path(__file__).with_name("migrations") / "008_seed_glenfiddich_infographic.sql"
        sql = sql_file.read_text(encoding="utf-8")
        match = re.search(r"\$glenfiddich_data\$\s*(.*?)\s*\$glenfiddich_data\$", sql, re.DOTALL)
        self.assertIsNotNone(match)
        payload = InfographicCreate(
            title="Glenfiddich History",
            type="timeline",
            distillery_id=self.distillery.id,
            schema_data=json.loads(match.group(1)),
        )
        self.assertEqual(len(payload.schema_data.steps), 12)
        self.assertEqual(payload.schema_data.steps[0].dateOrYear, "1886")
        self.assertEqual(payload.schema_data.steps[-1].dateOrYear, "2023–2026")
        for step in payload.schema_data.steps:
            for language in ("en", "ru", "uk"):
                self.assertTrue(getattr(step.title, language))
                self.assertTrue(getattr(step.description, language))

    def test_glengyle_sql_contains_valid_localized_timeline(self):
        sql_file = Path(__file__).with_name("migrations") / "009_seed_glengyle_infographic.sql"
        sql = sql_file.read_text(encoding="utf-8")
        match = re.search(r"\$glengyle_data\$\s*(.*?)\s*\$glengyle_data\$", sql, re.DOTALL)
        self.assertIsNotNone(match)
        payload = InfographicCreate(
            title="Glengyle History",
            type="timeline",
            distillery_id=self.distillery.id,
            schema_data=json.loads(match.group(1)),
        )
        self.assertEqual(len(payload.schema_data.steps), 14)
        self.assertEqual(payload.schema_data.steps[0].dateOrYear, "1872")
        self.assertEqual(payload.schema_data.steps[-1].dateOrYear, "2025–2026")
        for step in payload.schema_data.steps:
            for language in ("en", "ru", "uk"):
                self.assertTrue(getattr(step.title, language))
    ...
    def test_glenmorangie_sql_contains_valid_localized_timeline(self):
        sql_file = Path(__file__).with_name("migrations") / "010_seed_glenmorangie_infographic.sql"
        sql = sql_file.read_text(encoding="utf-8")
        match = re.search(r"\$glenmorangie_data\$\s*(.*?)\s*\$glenmorangie_data\$", sql, re.DOTALL)
        self.assertIsNotNone(match)
        payload = InfographicCreate(
            title="Glenmorangie History",
            type="timeline",
            distillery_id=self.distillery.id,
            schema_data=json.loads(match.group(1)),
        )
        self.assertEqual(len(payload.schema_data.steps), 18)
        self.assertEqual(payload.schema_data.steps[0].dateOrYear, "1703 / 1738")
        self.assertEqual(payload.schema_data.steps[-1].dateOrYear, "2024–2026")
        for step in payload.schema_data.steps:
            for language in ("en", "ru", "uk"):
                self.assertTrue(getattr(step.title, language))
